...
/**
 * External embedding provider supporting OpenAI, Gemini, and custom HTTP APIs.
 * 
 * Configuration examples:
 * - OpenAI: embedding.provider=external, embedding.external.type=openai
 * - Gemini: embedding.provider=external, embedding.external.type=gemini
 * - Custom: embedding.provider=external, embedding.external.type=http
 */
@Service
@Slf4j
...
public class ExternalEmbeddingProvider implements EmbeddingProvider {
    ...
    public ExternalEmbeddingProvider(ObjectMapper objectMapper) {
    ...
    private String buildTemplateBody(String text) {
        ...
        return requestTemplate.replace("${text}", escape(text));
    }
